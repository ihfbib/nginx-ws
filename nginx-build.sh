#!/usr/bin/env bash
# -------------------------------------------------------------------------
#  Nginx-ws - Automated Nginx compilation from source
# -------------------------------------------------------------------------
# Website:       https://www.wooserv.net
# GitHub:        https://github.com/ihfbib/nginx-ws
# Copyright (c) 2022 WooServ <contact@wooserv.net>
...
_help() {
    echo " -------------------------------------------------------------------- "
    echo "   Nginx-ws : automated Nginx compilation with additional modules  "
    echo " -------------------------------------------------------------------- "
    echo ""
    echo "Usage: ./nginx-ws <options> [modules]"
    echo "By default, Nginx-ws will compile the latest Nginx mainline release without Naxsi or RTMP module"
    echo "  Options:"
...
for package in $required_packages; do
    if ! command_exists "$package"; then
        apt install "$package" -qq >/dev/null 2>&1
    fi
done

# Checking if lsb_release is installed
if ! command_exists lsb_release; then
    apt -qq install lsb-release >/dev/null 2>&1
fi
...
NGINX_WS_VER=$(curl -m 5 --retry 3 -sL https://api.github.com/repos/ihfbib/nginx-ws/releases/latest 2>&1 | jq -r '.tag_name')
...
echo ""
echo "Welcome to the nginx-ws bash script ${NGINX_WS_VER}"
echo ""
...
if [ "$INTERACTIVE_SETUP" = "1" ]; then
    ...
    echo -e '\nDo you want to setup nginx-ws auto-update cronjob ? (y/n)'
...
_install_dependencies() {
    ...
    if {
        apt -o Dpkg::Options::="--force-confmiss" -o Dpkg::Options::="--force-confold" -y install \
    ...
    } >>/tmp/nginx-ws.log 2>&1; then
        echo -ne "       Installing dependencies                [${CGREEN}OK${CEND}]\\r"
        echo -ne '\n'
    else
        echo -e "        Installing dependencies              [${CRED}FAIL${CEND}]"
        echo -e '\n      Please look at /tmp/nginx-ws.log\n'
        exit 1
...
_nginx_from_scratch_setup() {
    ...
    if {
...

        {
            ...
            wget -O /var/www/html/index.nginx-debian.html https://raw.githubusercontent.com/ihfbib/nginx-ws/master/var/www/html/index.nginx-debian.html
            mkdir -p /etc/nginx/sites-enabled
            ln -s /etc/nginx/sites-available/default /etc/nginx/sites-enabled/
            # download nginx systemd service
            [ ! -f /lib/systemd/system/nginx.service ] && {
                wget -O /lib/systemd/system/nginx.service https://raw.githubusercontent.com/ihfbib/nginx-ws/master/etc/systemd/system/nginx.service
                systemctl enable nginx.service
            }

            # download logrotate configuration
            wget -O /etc/logrotate.d/nginx https://raw.githubusercontent.com/ihfbib/nginx-ws/master/etc/logrotate.d/nginx
...
_cleanup_modules() {
    ...
    cd "$DIR_SRC" || exit 1
    rm -rf /usr/local/src/{*.tar.gz,nginx,nginx-1.*,pcre,zlib,install,ngx_http_redis,naxsi}
...
_download_modules() {
    ...
    if {
        ...
        google/ngx_brotli ihfbib/nginx-upstream-fair '
...
_configure_nginx() {
    ...
    if {
        ...
        bash -c "./configure \
                    ...
                    --sbin-path=/usr/sbin/nginx >> /tmp/nginx-ws.log 2>&1;"
...
_cron_setup() {
    echo -ne '       Installing Nginx-ws Cronjob            [..]\r'
    if {
        wget -O /etc/cron.daily/nginx-ws https://raw.githubusercontent.com/ihfbib/nginx-ws/master/etc/cron.daily/nginx-ws >>/tmp/nginx-ws.log
        chmod +x /etc/cron.daily/nginx-ws

    }; then
        echo -ne "       Installing Nginx-ws Cronjob            [${CGREEN}OK${CEND}]\\r"
        echo -ne '\n'
    else
        echo -e "       Installing Nginx-ws Cronjob            [${CRED}FAIL${CEND}]"
        echo -e '\n      Please look at /tmp/nginx-ws.log\n'
        exit 1
    fi
...
_cron_update() {
    if [ -f /etc/cron.daily/nginx-ws ]; then
        wget -O /etc/cron.daily/nginx-ws https://raw.githubusercontent.com/ihfbib/nginx-ws/master/etc/cron.daily/nginx-ws >>/tmp/nginx-ws.log
        chmod +x /etc/cron.daily/nginx-ws
    fi
}
...
_final_tasks() {
    ...
    if [ "$NOCONF" != "y" ]; then
        ...
        if [ -z "$VERIFY_NGINX_CONFIG" ]; then
            ...
            echo ""
            echo -e "       ${CGREEN}Nginx-ws was compiled successfully !${CEND}"
            echo -e '\n       Installation log : /tmp/nginx-ws.log\n'
        else
            echo -e "       Checking nginx configuration           [${CRED}FAIL${CEND}]"
            echo -e "       Nginx-ws was compiled successfully but there is an error in your nginx configuration"
            echo -e '\nPlease look at /tmp/nginx-ws.log or use the command nginx -t to find the issue\n'
        fi
    else
        echo -e "       ${CGREEN}Nginx-ws was compiled successfully !${CEND}"
...
_final_tasks
echo "Give Nginx-ws a GitHub star : https://github.com/ihfbib/nginx-ws"
